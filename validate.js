...
/* ---------------- 12. full CH52–CH54 playthroughs (Bleeding Disorders + Resuscitation + Shock) ---------------- */
const NEW_CH4 = { 52: [10, 176], 53: [6, 120], 54: [7, 154] };
for (const ch of Object.keys(NEW_CH4)) {
  const [uN, qN] = NEW_CH4[ch];
  const cu = A.unitsOf(+ch);
  ok(cu.length === uN, `CH${ch} has ${uN} units (${cu.length})`);
  const qTotal = cu.reduce((n, u) => n + u.qs.length, 0);
  ok(qTotal === qN, `CH${ch} has ${qN} questions (${qTotal})`);
...
console.log('— after CH52–CH54'); ok(A.CHAPTERS.filter(c => c.live).length === LIVE, 'live chapter count unchanged after the CH52–CH54 playthroughs');
ok(LIVE === 54, `54 live chapters registered (${LIVE})`);
ok(A.QUESTIONS.length === 6445, `bank holds 6445 questions (${A.QUESTIONS.length})`);
ok(A.UNITS.length === 442, `bank holds 442 units (${A.UNITS.length})`);
